...
- Catálogo de correos: `lib/emails.ts` — array `EMAILS` + helpers `S()` / `SP()`.
- Motor de secciones: `lib/email-templates.ts` — union `Section['type']`, `renderSection()`, `generateEmail()`.
- Tokens de color y tipografía: `patrones.md` (raíz del repo).
- CDN de íconos conocidos (referencia desde `emails.ts`):
  - `icon-calendar.png` · `icon-search.png` · `icon-eye.png`
  - `icon-cards.png` · `icon-bank-building.png` · `phone-bcp.png`
  - Base: `https://cdn.vmcsubastas.com/services/boletin/assets/`
